...
async function updateRoom(updatedRoom) {
    if (!objectId) return null;
    const url = `${BmobApiBase}/GameRoom/${objectId}`;
    return await bmobRequest(url, 'PUT', updatedRoom);
}
...
// 初始化网络游戏
async function initNetworkGame() {
    if (!roomId) return;
...
async function handlePowerClick(e) {
    if (gameState.phase !== 'power') return;
    ...
    await moveBadge(distance);
}

async function moveBadge(distance) {
    // 播放移动音效
    playAudio('jump');
    ...
    async function animateMove(timestamp) {
...
async function checkGoldenTile() {
    const player = gameState.players[gameState.currentPlayer - 1];
